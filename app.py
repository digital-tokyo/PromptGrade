...
st.markdown("""
    ...
    <div class="pg-sub">SaaS-grade evaluation for your AI prompts—clarity, structure, and actionable feedback.</div>
...
with st.container():
    ...
    with col2:
        ...
        with st.expander("Advanced", expanded=False):
            st.markdown("- **Confidential**: No prompts are saved.\n- **Scoring**: Rubric is clarity, politeness, and completeness.")
        st.info("Grading uses a 3-point rubric: Clarity, Specificity, and Instruction.")
        ...
        if st.button("⚡ Grade My Prompt", help="Atomic evaluation: see score and actionable feedback"):
            if prompt.strip() == "":
                ...
                with st.spinner("Grading in progress..."):
                    sleep(0.3)
                    grade, feedback = grade_prompt(prompt)
                    report = generate_report(prompt, grade, feedback)

                st.markdown(f"""
                    <div class="pg-report-zone">
                        <h3 style='margin-top:0;'>Result: <span style="font-family:monospace;font-size:1.34em;color:#2174AB;">{grade}</span></h3>
                        <pre style="white-space:pre-wrap;background:none;border:none;padding:0;color:#2E4157;font-size:1.09em;">{feedback}</pre>
                    </div>
                """, unsafe_allow_html=True)
                ...
                st.download_button(
                    ...
                    data=report,
                    file_name="PromptGrade_Report.txt",
                    mime="text/plain"
                )

st.caption("Built with ❤️ and atomic precision. UI inspired by Linear/Stripe/Notion. Accessibility: 100% keyboard/tab navigation.")
